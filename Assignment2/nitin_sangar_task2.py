...
firstRow = data.first()
basketsPerUsers = data.filter(lambda row: row != firstRow).map(lambda x: x.split(",")).groupByKey().mapValues(lambda x : list(x))
...
def apriori(iterator, support, totalBaskets):
   ...
   # print("chunk baskets is {}".format(chunkBaskets))
...
def task(k, support):
    filteredUsers = basketsPerUsers.filter(lambda x:len(x[1])>k).mapValues(lambda x:set(x))
    businessBaskets = filteredUsers.values()
    totalBaskets = businessBaskets.count()
   # print("data is loaded")
    candidateList = businessBaskets.mapPartitions(lambda x: apriori(x, support, totalBaskets)).distinct()
    ...
    frequentItemsets = businessBaskets.mapPartitions(lambda x: apriori1(x, candidateList)).reduceByKey(lambda x,y:x+y).filter(lambda x: x[1]>=support).keys()
